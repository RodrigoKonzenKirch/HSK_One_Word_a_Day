...
class ChineseWordDaoTest {
    ...
    fun insertAndGetWord() = runTest {
        val word = ChineseWordEntity(1, "hsk1", "一", "一", "yi", "yi1", "one")
    ...
    @Test
    fun insertAndGetWordCount() = runTest {
        val words = listOf(
            ChineseWordEntity(1, "hsk1", "一", "一", "yi", "yi1", "one"),
            ChineseWordEntity(2, "hsk1", "二", "二", "er", "er2", "two"),
            ChineseWordEntity(3, "hsk1", "三", "三", "san", "san3", "three")
        ...
        chineseWordDao.insertAll(words)
        val wordCount = chineseWordDao.getWordCount()

        assertThat(wordCount).isEqualTo(words.size)
    }

    @Test
    fun insertAndGetWordCountZero() = runTest {
        val wordCount = chineseWordDao.getWordCount()
